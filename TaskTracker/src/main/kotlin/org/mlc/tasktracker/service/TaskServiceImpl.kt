...
class TaskServiceImpl(
  ...
  override fun createTask(
    ...
    val user =
      userRepository.findById(userId).orElse(null) ?: throw UserNotFoundException("User with ID $userId not found")
  ...
  override fun getTaskById(userId: String, taskId: String): TaskResponseDTO? {
    return taskRepository.findById(taskId).orElse(null)?.let { task ->
  ...
  override fun deleteTask(userId: String, taskId: String): Boolean {
    taskRepository.findById(taskId).orElse(null)?.let { taskToDelete ->
