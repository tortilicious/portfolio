package org.mlc.tasktracker.service // Package corrected for general service interfaces
...
interface TaskListService {
   ...
   * Returns an empty list if the user has no task lists or if the user is not found.
   */
   ...
   * @return A [TaskListResponseDTO] representing the found task list if it exists and belongs to the user;
   * otherwise, `null`.
   */
   ...
   * @return A [TaskListResponseDTO] representing the updated task list if the operation is successful;
   * `null` if the task list is not found or if the user is not authorized to update it.
   * @throws IllegalArgumentException if the [updateRequestDTO] is invalid.
   */
  fun updateTaskList(userId: String, taskListId: String, updateRequestDTO: UpdateTaskListRequestDTO): TaskListResponseDTO?
   ...
   * @return `true` if the task list was successfully found and deleted; `false` if the task list was not found
   * or if the user is not authorized to delete it.
   */
